...
platform :ios do
  ...
  lane :upload_metadata do
    ...
    begin
      ...
      # fastlane ≤ 2.233.1 bug: deliver always calls fetch_app_store_review_detail
      # even when not submitting for review. For new apps that have no review detail
      # record yet in App Store Connect, the API returns empty data → "No data" crash.
      # Text metadata (names, descriptions, keywords) is uploaded BEFORE this call,
      # so the actual metadata content has already been sent successfully.
      # Fix: https://github.com/fastlane/fastlane/issues/20538
      raise e unless e.message.to_s.include?("No data")
      UI.important("")
      UI.important("⚠️  Caught known fastlane bug: 'No data' from fetch_app_store_review_detail.")
      UI.important("   This happens for new apps with no review detail record in App Store Connect.")
      UI.important("   Text metadata (names, descriptions, keywords) was already uploaded.")
      UI.important("   Please verify the upload in App Store Connect → App Information.")
      UI.important("   The fix will land when Apple creates the review detail automatically")
      UI.important("   after the first binary upload, or when fastlane issue #20538 is resolved.")
      UI.important("")
    end
  end
